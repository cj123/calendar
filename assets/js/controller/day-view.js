angular.module("calendar")
    .controller("DayView", ["$scope", "$uibModal", "Appointment", "CalendarData", "moment", function($scope, $uibModal, Appointment, CalendarData, moment) {
        ...
        function loadAppointments(date) {
            ...
            Appointment.getAppointments(date).then(function(response) {
                ...
                for (var appointmentIndex = 0; appointmentIndex < appointments.length; appointmentIndex++) {
                    ...
                    var startTime = $scope.currentDate.clone().minute(0).hour(0).add(appointment.start, "minutes");
